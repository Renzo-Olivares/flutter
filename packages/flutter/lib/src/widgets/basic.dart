...
class RichText extends MultiChildRenderObjectWidget {
  ...
  @override
  MultiChildRenderObjectElement createElement() => _RichTextElement(this);

  @override
  RenderParagraph createRenderObject(BuildContext context) {
    assert(textDirection != null || debugCheckHasDirectionality(context));
    final element = context as _RichTextElement;
    element._configureSelection();
    return RenderParagraph(
      ...
      registrar: element._effectiveRegistrar,
...
/// The element used by [RichText] to host the paragraph-aware
/// [SelectionContainerDelegate] that coordinates selection across the
/// [_SelectableFragment]s split by [PlaceholderSpan]s and the [Selectable]s
/// contributed by [WidgetSpan] children.
///
/// When this element is mounted under an ambient [SelectionRegistrar] (or when
/// [RichText.selectionRegistrar] is non-null), it lazily creates a
/// [_SelectableTextContainerDelegate] and uses it as the [RenderParagraph]'s
/// registrar. Each [WidgetSpan] child is wrapped in a [SelectionRegistrarScope]
/// pointing at that delegate, so any [Selectable] inside the [WidgetSpan]
/// content registers with the same delegate as the paragraph fragments. A
/// single proxy [Selectable] is registered with the outer registrar to
/// represent the whole [RichText] as one coherent group.
class _RichTextElement extends MultiChildRenderObjectElement {
  _RichTextElement(RichText super.widget);

  _SelectableTextContainerDelegate? _delegate;
  _RichTextSelectableProxy? _proxy;

  RichText get _widget => widget as RichText;

  /// The registrar passed to [RenderParagraph]. When the auto-wrap path is
  /// active this is the delegate; otherwise it's whatever the user passed via
  /// [RichText.selectionRegistrar].
  SelectionRegistrar? get _effectiveRegistrar => _delegate ?? _widget.selectionRegistrar;

  /// Called from [RichText.createRenderObject] and [RichText.updateRenderObject]
  /// before the [RenderParagraph]'s registrar is computed. Sets up the delegate
  /// the first time an outer registrar is observed.
  void _configureSelection() {
    if (_delegate != null) {
      return;
    }
    final SelectionRegistrar? outer =
        _widget.selectionRegistrar ?? SelectionContainer.maybeOf(this);
    if (outer == null) {
      return;
    }
    _delegate = _SelectableTextContainerDelegate(this);
  }

  void _updateProxyRegistration() {
    if (_delegate == null) {
      return;
    }
    final SelectionRegistrar? outer =
        _widget.selectionRegistrar ?? SelectionContainer.maybeOf(this);
    if (outer == null) {
      _proxy?.registrar = null;
      return;
    }
    _proxy ??= _RichTextSelectableProxy(element: this, delegate: _delegate!);
    _proxy!.registrar = outer;
  }

  @override
  void mount(Element? parent, Object? newSlot) {
    // super.mount calls RichText.createRenderObject (which calls
    // _configureSelection) and then inflates the children, so by the time it
    // returns the delegate exists if it's needed and the children have been
    // wrapped accordingly.
    super.mount(parent, newSlot);
    _updateProxyRegistration();
  }

  @override
  void update(covariant RichText newWidget) {
  ...
  Element inflateWidget(Widget newWidget, Object? newSlot) {
    ...
    return super.inflateWidget(
      SelectionRegistrarScope(registrar: _delegate!, child: newWidget),
      newSlot,
    );
  }
  ...
  List<Element> updateChildren(
    ...
    final wrapped = <Widget>[
      for (final w in newWidgets) SelectionRegistrarScope(registrar: _delegate!, child: w),
    ];
    return super.updateChildren(
      oldChildren,
      wrapped,
  ...
  @override
  void unmount() {
    // Dispose the proxy first so it unregisters from the outer registrar
    // before the underlying RenderParagraph is torn down.
    _proxy?.dispose();
    _proxy = null;
    // super.unmount disposes the RenderParagraph, which calls
    // _registrar.remove(fragment) for each fragment. The registrar at that
    // point is _delegate, so the delegate must still be alive.
    super.unmount();
    _delegate?.dispose();
    _delegate = null;
  }
...
/// The proxy [Selectable] registered with the outer registrar on behalf of a
/// [_RichTextElement]. Forwards everything to the element's delegate and uses
/// the [RenderParagraph] for geometry.
class _RichTextSelectableProxy extends ChangeNotifier with Selectable, SelectionRegistrant {
  _RichTextSelectableProxy({
    required _RichTextElement element,
  ...
  final _RichTextElement _element;
  final SelectionContainerDelegate _delegate;

  RenderBox get _renderBox => _element.renderObject as RenderBox;

  @override
  SelectionGeometry get value => _delegate.value;

  @override
  SelectionResult dispatchSelectionEvent(SelectionEvent event) =>
      _delegate.dispatchSelectionEvent(event);

  @override
  SelectedContent? getSelectedContent() => _delegate.getSelectedContent();

  @override
  SelectedContentRange? getSelection() => _delegate.getSelection();

  @override
  void pushHandleLayers(LayerLink? startHandle, LayerLink? endHandle) =>
      _delegate.pushHandleLayers(startHandle, endHandle);

  @override
  Matrix4 getTransformTo(RenderObject? ancestor) => _renderBox.getTransformTo(ancestor);
  ...
  @override
  List<Rect> get boundingBoxes => <Rect>[_renderBox.paintBounds];
...
/// Coordinates selection across the [_SelectableFragment]s of a single
/// [RenderParagraph] and the [Selectable]s contributed by its [WidgetSpan]
/// placeholders. Triple-click extends a paragraph across fragments by
/// synthesizing absorb events for the placeholders between them, the screen
/// order comparator places fragments and placeholders by paint position
/// rather than registration order, and [getSelection] aggregates the
/// fragment-and-placeholder ranges into a single content range relative to
/// the paragraph's full text.
class _SelectableTextContainerDelegate extends StaticSelectionContainerDelegate {
  _SelectableTextContainerDelegate(this._element);

  final _RichTextElement _element;

  /// The [RenderParagraph] this delegate coordinates selection for.
  RenderParagraph get paragraph => _element.renderObject as RenderParagraph;

  RenderBox get _container => _element.renderObject as RenderBox;

  // [SelectionContainerDelegate]'s default implementations of the geometry
  // helpers below depend on `_selectionContainerContext`, which is private to
  // selection_container.dart and only set when a [SelectionContainer] widget
  // wraps the delegate. _RichTextElement intentionally avoids that wrapper, so
  // we route the helpers through the [RenderParagraph] directly.

  @override
  Matrix4 getTransformFrom(Selectable child) {
    return child.getTransformTo(_container);
  }

  @override
  Matrix4 getTransformTo(RenderObject? ancestor) {
    return _container.getTransformTo(ancestor);
  }

  @override
  bool get hasSize => _container.hasSize;

  @override
  Size get containerSize => _container.size;
  ...
  SelectedContentRange? _calculateLocalRange(List<_SelectionInfo> selections) {
    ...
    for (var index = 0; index < selections.length; index++) {
      ...
      final int selectionStartNormalized = math.min(
        selection.range!.startOffset,
        selection.range!.endOffset,
      );
      final int selectionEndNormalized = math.max(
        selection.range!.startOffset,
        selection.range!.endOffset,
      );
      if (!foundStart) {
  ...
  void _flushInactiveSelections() {
    ...
    final int skipStart = math.min(currentSelectionStartIndex, currentSelectionEndIndex);
    final int skipEnd = math.max(currentSelectionStartIndex, currentSelectionEndIndex);
    for (var index = 0; index < selectables.length; index += 1) {
