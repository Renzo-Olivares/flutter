...
> 2. **Platform-Specific Dispatch**: The widget's gesture handlers interpret single taps, double taps, and triple taps as caret/selection, word, or line/paragraph operations. In `TextSelectionGestureDetectorBuilder`, desktop single-tap selection occurs on tap down; ordinary Android, Fuchsia, and iOS single-tap selection occurs on tap up. The recognizer also has a tap-down deadline, so tap counting alone does not imply instantaneous pointer-down caret placement.
