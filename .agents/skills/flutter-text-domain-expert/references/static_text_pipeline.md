...
- **Role of `_selectionStartsInScrollable` & Drag Boundary Clamping**:
  - For pointer-driven selection, `_selectionStartsInScrollable` tracks whether the drag selection originated inside or outside this scrollable's viewport.
  - **Purpose in plain English**: A selection that starts outside the scrollable can sweep across it as part of a larger document selection, selecting its contents together with a "select all" effect, without autoscrolling that scrollable. A selection that starts inside the scrollable can instead extend progressively through its contents: dragging beyond the viewport edge activates autoscrolling when more content is available.
  - Starting outside does not mean that merely entering the viewport selects everything. While the dragged endpoint is inside, selection can still end at a specific point in the content; the whole-container effect comes from extending the surrounding selection across its boundaries.
